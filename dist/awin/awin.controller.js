    ...
    async getAllProducts(page = '1', limit = '50', category, subs, search, colors, sizes, materials, merchants, types, minPrice, maxPrice) {
        ...
        if (search) {
            ...
            const targetCats = allCats.filter(c => c.slug.toLowerCase() === category.toLowerCase() ||
                c.name.toLowerCase() === category.toLowerCase());
            console.log(...oo_oo(`3069100425_550_6_550_105_4`, `[getAllProducts] Query: "${category}", Found ${targetCats.length} target categories.`));
            const getDescendantIds = (catId, visited = new Set()) => {
                if (visited.has(catId))
                    return [];
                visited.add(catId);
            ...
            console.log(...oo_oo(`3069100425_621_6_621_114_4`, `[getAllProducts] Query: "${category}", IDs: ${uniqueIds.length}, Names: ${uniqueNames.length}`));
            where.OR = [
            ...
            let isLightingCategory = false;
            console.log(...oo_oo(`3069100425_639_6_639_69_4`, `[isLightingCategory Check] category: ${category}`));
            console.log(...oo_oo(`3069100425_640_6_640_130_4`, `[isLightingCategory Check] targetCats:`, targetCats.map(c => ({ id: c.id, name: c.name, parentId: c.parentId }))));
            if (category && category.toLowerCase() === 'lighting') {
                isLightingCategory = true;
            }
            else {
                for (const cat of targetCats) {
                    let currentCat = cat;
                    while (currentCat) {
                        console.log(...oo_oo(`3069100425_647_12_647_81_4`, `[isLightingCategory Check] tracing: ${currentCat.name}`));
                        if (currentCat.name.toLowerCase() === 'lighting' || currentCat.slug.toLowerCase() === 'lighting') {
                            isLightingCategory = true;
                            break;
                        }
            ...
            console.log(...oo_oo(`3069100425_657_6_657_77_4`, `[isLightingCategory Check] result: ${isLightingCategory}`));
            if (isLightingCategory) {
                const nonLightingTerms = [
                    'chair', 'sofa', 'stool', 'bench', 'dining table', 'side table',
                    'coffee table', 'console table', 'dressing table', 'wardrobe',
        ...
        if (total === 0 && category && category !== 'all-products' && !hasFilters) {
            console.log(...oo_oo(`3069100425_780_6_780_100_4`, `[getAllProducts] No products found for "${category}". Trying parent fallback...`));
            ...
            if (allCats && categoryMap) {
                ...
                if (target && target.parentId) {
                    ...
                    if (parent) {
                        console.log(...oo_oo(`3069100425_788_12_788_91_4`, `[getAllProducts] Falling back to parent category: ${parent.name}`));
