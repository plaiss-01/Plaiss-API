...
let AwinController = AwinController_1 = class AwinController {
    ...
    async getFacets(category, subs) {
        ...
        const targetCats = category ? allCats.filter(c => c.slug.toLowerCase() === category.toLowerCase() ||
    ...
    async getAllProducts(page = '1', limit = '50', category, subs, search, colors, sizes, materials, merchants, types, minPrice, maxPrice) {
        ...
        if (search) {
            ...
            const targetCats = allCats.filter(c => c.slug.toLowerCase() === category.toLowerCase() ||
                c.name.toLowerCase() === category.toLowerCase());
            console.log(...oo_oo(`2273497171_607_6_607_105_4`, `[getAllProducts] Query: "${category}", Found ${targetCats.length} target categories.`));
            const getDescendantIds = (catId, visited = new Set()) => {
                if (visited.has(catId))
                    return [];
                visited.add(catId);
            ...
            console.log(...oo_oo(`2273497171_688_6_688_114_4`, `[getAllProducts] Query: "${category}", IDs: ${uniqueIds.length}, Names: ${uniqueNames.length}`));
            where.OR = [
            ...
            let isLightingCategory = false;
            console.log(...oo_oo(`2273497171_706_6_706_69_4`, `[isLightingCategory Check] category: ${category}`));
            console.log(...oo_oo(`2273497171_707_6_707_130_4`, `[isLightingCategory Check] targetCats:`, targetCats.map(c => ({ id: c.id, name: c.name, parentId: c.parentId }))));
            if (category && category.toLowerCase() === 'lighting') {
                isLightingCategory = true;
            }
            else {
                for (const cat of targetCats) {
                    let currentCat = cat;
                    while (currentCat) {
                        console.log(...oo_oo(`2273497171_714_12_714_81_4`, `[isLightingCategory Check] tracing: ${currentCat.name}`));
                        if (currentCat.name.toLowerCase() === 'lighting' || currentCat.slug.toLowerCase() === 'lighting') {
                            isLightingCategory = true;
                            break;
                        }
            ...
            console.log(...oo_oo(`2273497171_724_6_724_77_4`, `[isLightingCategory Check] result: ${isLightingCategory}`));
            if (isLightingCategory) {
                const nonLightingTerms = [
                    'chair', 'sofa', 'stool', 'bench', 'dining table', 'side table',
                    'coffee table', 'console table', 'dressing table', 'wardrobe',
        ...
        if (total === 0 && category && category !== 'all-products' && !hasFilters) {
            console.log(...oo_oo(`2273497171_847_6_847_100_4`, `[getAllProducts] No products found for "${category}". Trying parent fallback...`));
            ...
            if (allCats && categoryMap) {
                ...
                if (target && target.parentId) {
                    ...
                    if (parent) {
                        console.log(...oo_oo(`2273497171_855_12_855_91_4`, `[getAllProducts] Falling back to parent category: ${parent.name}`));
