...
let AwinService = AwinService_1 = class AwinService {
    ...
    rawInsertBatchSize = 500;
    validSizeLabels = new Set([
        '1 Seater',
        '2 Seater',
        '3 Seater',
        '4 Seater',
    ...
    async extractCsvFileToRaw(fileBuffer, jobId, replace = true) {
        await this.ensureAwinPipelineTables();
        if (replace) {
            await this.prisma.$executeRawUnsafe(`TRUNCATE TABLE "${this.awinPipelineTables.raw}"`);
        }
        const parser = stream_1.Readable.from(fileBuffer).pipe(csv.parse({ headers: true }));
        let count = 0;
        let batch = [];
        for await (const row of parser) {
            count++;
    ...
    async transformRawToDev(replace = true, jobId) {
        await this.ensureAwinPipelineTables();
        if (replace) {
            await this.prisma.$executeRawUnsafe(`TRUNCATE TABLE "${this.awinPipelineTables.dev}"`);
        }
        const [{ count: rawCount }] = await this.prisma.$queryRawUnsafe(`SELECT COUNT(*)::int AS count FROM "${this.awinPipelineTables.raw}"`);
        const rawTotal = Number(rawCount) || 0;
        if (jobId) {
            this.statusService.updateJob(jobId, 0, `Found ${rawTotal} RAW rows. Reading the first RAW batch...`, rawTotal || 1);
        }
        let transformed = 0;
        let skipped = 0;
        const mappedRows = [];
        ...
        while (rawProcessed < rawTotal) {
            const rows = await this.prisma.$queryRawUnsafe(`SELECT row_number, raw_row FROM "${this.awinPipelineTables.raw}"
         WHERE row_number > $1
         ORDER BY row_number ASC
         LIMIT $2`, lastRowNumber, batchSize);
            if (rows.length === 0)
                break;
            ...
            if (jobId) {
                this.statusService.updateJob(jobId, rawProcessed, `Mapped ${rawProcessed} of ${rawTotal} RAW rows. ${mappedRows.length} valid products found...`, rawTotal || rows.length || 1);
            }
        ...
        if (jobId) {
            this.statusService.updateJob(jobId, rawProcessed, `Saving ${mappedRows.length} transformed DEV rows...`, totalWork || 1);
        }
      ...
      `;
        ...
        if (jobId) {
            this.statusService.completeJob(jobId, `DEV transform complete: ${transformed} transformed, ${skipped} skipped.`, result);
        }
        return result;
    }
    ...
    getAllProductImageCandidates(product, size = 900) {
        let rawData = {};
        if (product.rawRow) {
            try {
        ...
        const candidates = [];
        const addUnique = (val) => {
            if (!this.isUsableImageValue(val))
                return;
            const secure = val.trim().replace(/^http:\/\//i, 'https://');
            if (secure.includes('noimage.gif'))
                return;
        ...
        for (const source of rawCandidates) {
            if (!this.isUsableImageValue(source))
                continue;
    ...
    async verifyAndUpdateProdImages(jobId) {
        ...
        for (let i = 0; i < products.length; i += chunkSize) {
            ...
            await Promise.all(chunk.map(async (p) => {
                ...
                if (validUrl && validUrl !== p.imageUrl) {
                    await this.prisma.$executeRawUnsafe(`UPDATE "AWIN_AFFILIAT_PRODUCTS_DATA_PROD" SET image_url = $1 WHERE aw_product_id = $2`, validUrl, p.id);
                }
            }));
            if (jobId && i % 100 === 0) {
        ...
        const variants = await this.prisma.productColorVariant.findMany({
            select: {
                id: true,
                imageUrl: true,
            },
            ...
            await Promise.all(chunk.map(async (v) => {
                if (v.imageUrl && !(await checkImageUrl(v.imageUrl))) {
                    ...
                    if (await checkImageUrl(proxyUrl)) {
                        await this.prisma.$executeRawUnsafe(`UPDATE "ProductColorVariant" SET image_url = $1 WHERE id = $2`, proxyUrl, v.id);
                    }
                }
            }));
        }
    ...
    `);
        ...
        if (jobId) {
            this.statusService.completeJob(jobId, `PROD promotion complete: ${count} PROD rows, ${syncedProducts} products synced.`, result);
        }
        return result;
    }
    withAwinDownloadDefaults(url) {
        if (url.includes('datafeed/download') && !url.includes('/columns/') && !url.includes('columns=')) {
            if (url.includes('download.php')) {
                const separator = url.includes('?') ? '&' : '?';
    ...
    async ensureAwinPipelineTables() {
        await this.prisma.$executeRawUnsafe(`
    ...
    `);
        await this.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "idx_awin_raw_row_number"
       ON "${this.awinPipelineTables.raw}" (row_number)`);
        await this.prisma.$executeRawUnsafe(this.createPipelineProductTableSql(this.awinPipelineTables.dev, false));
        await this.prisma.$executeRawUnsafe(this.createPipelineProductTableSql(this.awinPipelineTables.prod, true));
    ...
    async countAwinPipelineRows(tableName) {
        const [{ count }] = await this.prisma.$queryRawUnsafe(`SELECT COUNT(*)::int AS count FROM "${tableName}"`);
        return Number(count) || 0;
    }
    async yieldToEventLoop() {
        await new Promise((resolve) => setImmediate(resolve));
    }
    async ensurePipelineProductColumns(tableName, includeLoadedAt) {
        const columns = [
        ...
        for (const [column, type] of columns) {
            await this.prisma.$executeRawUnsafe(`ALTER TABLE "${tableName}" ADD COLUMN IF NOT EXISTS ${column} ${type}`);
        }
    }
    createPipelineProductTableSql(tableName, includeLoadedAt) {
        return `
    ...
    async insertRawAwinRow(row, rowNumber, sourceUrl, jobId) {
        await this.prisma.$executeRawUnsafe(`INSERT INTO "${this.awinPipelineTables.raw}" (id, row_number, source_url, import_job_id, raw_row)
       VALUES ($1, $2, $3, $4, $5::jsonb)`, (0, crypto_1.randomUUID)(), rowNumber, sourceUrl, jobId || null, JSON.stringify(row));
    }
    async insertRawAwinRows(rows, sourceUrl, jobId) {
        if (rows.length === 0)
            return;
        const values = [];
        const placeholders = rows.map(({ row, rowNumber }, index) => {
            const offset = index * 5;
            values.push((0, crypto_1.randomUUID)(), rowNumber, sourceUrl, jobId || null, JSON.stringify(row));
            return `($${offset + 1}, $${offset + 2}, $${offset + 3}, $${offset + 4}, $${offset + 5}::jsonb)`;
        });
        await this.prisma.$executeRawUnsafe(`INSERT INTO "${this.awinPipelineTables.raw}" (id, row_number, source_url, import_job_id, raw_row)
       VALUES ${placeholders.join(', ')}`, ...values);
    }
    mapAwinRawRowToPipelineRow(row) {
    ...
    async upsertPipelineProductRow(tableName, row) {
        await this.prisma.$executeRawUnsafe(`INSERT INTO "${tableName}" (
        aw_product_id, merchant_product_id, product_name, slug, description, search_price,
        currency, image_url, product_url, merchant_name, category_name, merchant_category,
        category_id, brand_name, colour, product_model, product_type, product_model_clean,
        colour_clean, size_stock_status_clean, is_recliner, is_sofa_bed, base_sku,
      ...
      ON CONFLICT (aw_product_id) DO UPDATE SET
        ...
        sales_discount = EXCLUDED.sales_discount,
        raw_row = EXCLUDED.raw_row,
        transformed_at = NOW()`, row.awProductId, row.merchantProductId || null, row.productName, row.slug, row.description || null, row.price, row.currency || null, row.imageUrl, row.productUrl || null, row.merchantName || null, row.categoryName, row.merchantCategory || null, row.categoryId || null, row.brandName || null, row.colour || null, row.productModel || null, row.productType || null, row.productModelClean || null, row.colourClean || null, row.sizeStockStatusClean || null, row.isRecliner || null, row.isSofaBed || null, row.baseSku || null, row.colourVariantNumber || null, row.originalPriceClean, row.discountedPriceClean, row.saving, row.salesDiscount || null, JSON.stringify(row.rawRow));
    }
    async syncProductModelFromAwinProd(jobId, progressOffset = 0, progressTotal) {
        const rows = await this.prisma.$queryRawUnsafe(`SELECT * FROM "${this.awinPipelineTables.prod}" ORDER BY loaded_at DESC`);
        let synced = 0;
        ...
        for (let i = 0; i < rows.length; i += insertBatchSize) {
            ...
            for (const row of chunk) {
                let catRec = categoryCache.get(row.category_name.toLowerCase().trim());
    ...
    combineAwinFields(getVal, fields) {
        return fields
    ...
    inferAwinProductModel(row, getVal, categoryName) {
        ...
        for (const field of fields) {
            ...
            if (hasFabric && !hasLeather)
                return 'Fabric';
            if (hasLeather && !hasFabric)
                return 'Leather';
            if (hasFabric && hasLeather)
                return 'Review';
        }
    ...
    inferAwinSizeStockStatus(row, getVal, categoryName) {
        ...
        if (existing) {
            ...
            if (this.validSizeLabels.has(existingClean))
    ...
    inferAwinIsRecliner(row, getVal) {
        const text = this.combineAwinFields(getVal, [
            'product_name',
            'name',
            'description',
    ...
    extractBaseSkuFromAwinRow(row) {
        ...
        if (olMatch)
            return olMatch[1].toUpperCase();
        const pathProductMatch = text.match(/-p-(\d+)/i);
        if (pathProductMatch)
            return pathProductMatch[1];
    ...
    addVariantNumbers(rows) {
        const variantsBySku = new Map();
        for (const row of rows) {
            if (row.baseSku === 'Unknown' || row.colourClean === 'Unknown')
                continue;
            const colours = variantsBySku.get(row.baseSku) || new Set();
    ...
    async deduplicateProducts() {
        ...
        for (const [key, products] of groups.entries()) {
            ...
            const masterColor = master.colour || master.name.split(' ').find((word) => ['red', 'blue', 'green', 'black', 'white', 'grey', 'gray', 'yellow', 'pink', 'purple', 'brown', 'beige', 'cream', 'teal', 'navy', 'charcoal', 'silver', 'gold', 'orange', 'mocha', 'tan', 'saddle', 'ash', 'stone', 'latte', 'azul', 'slate', 'chalk', 'pistachio', 'amber', 'copper', 'brass', 'bronze', 'opal', 'bordeaux', 'rust', 'plum', 'burgundy', 'terracotta', 'mint', 'ivory', 'espresso', 'chocolate', 'khaki', 'denim', 'natural', 'steel', 'taupe', 'sand', 'ochre', 'mustard', 'emerald', 'sage', 'olive'].includes(word.toLowerCase()));
            if (masterColor)
                seenColors.add(masterColor.toLowerCase());
            for (const v of variants) {
                const colorName = v.colour || v.name.split(' ').find((word) => ['red', 'blue', 'green', 'black', 'white', 'grey', 'gray', 'yellow', 'pink', 'purple', 'brown', 'beige', 'cream', 'teal', 'navy', 'charcoal', 'silver', 'gold', 'orange', 'mocha', 'tan', 'saddle', 'ash', 'stone', 'latte', 'azul', 'slate', 'chalk', 'pistachio', 'amber', 'copper', 'brass', 'bronze', 'opal', 'bordeaux', 'rust', 'plum', 'burgundy', 'terracotta', 'mint', 'ivory', 'espresso', 'chocolate', 'khaki', 'denim', 'natural', 'steel', 'taupe', 'sand', 'ochre', 'mustard', 'emerald', 'sage', 'olive'].includes(word.toLowerCase())) || 'Original';
    ...
    async getHomepageProducts() {
        const selected = await this.prisma.homepageProduct.findMany({
            select: { productId: true }
        });
        const productIds = selected.map((s) => s.productId);
    ...
    async addHomepageProduct(productId) {
        try {
            const created = await this.prisma.homepageProduct.create({
                data: { productId }
            });
    ...
    async removeHomepageProduct(productId) {
        try {
            await this.prisma.homepageProduct.delete({
                where: { productId }
            });
            return { success: true };
        }
