...
(async () => {
  ...
  // Verify table exists
  const check = await client.query(`SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='public' AND table_name='Product'`);
  console.log('Product table exists:', check.rows[0].count);
  ...
  await client.query(`
    ...
    ON public."Product" USING GIN (category_name gin_trgm_ops)
  `);
  console.log('Index 1/3 done: category_name');

  await client.query(`
    CREATE INDEX IF NOT EXISTS "Product_merchant_category_trgm_idx"
    ON public."Product" USING GIN (merchant_category gin_trgm_ops)
  `);
