async function test() {
  const res = await fetch('http://localhost:3001/api/awin/products?category=lighting&limit=20');
  ...
  try {
    ...
    console.log('Retailers found in first 20 results:', Array.from(retailers));
    ...
    if (bad) {
      ...
      console.log('No bad products found in first 20 results.');
    }
  } catch (e) {
